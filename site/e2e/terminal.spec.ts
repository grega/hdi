...
test.describe('Terminal simulator', () => {
  ...
  test('unknown command shows error', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    await expect(page.locator('.t-line', { hasText: 'Command not found: foo' })).toBeVisible();
  });
  ...
  test('clear resets terminal and accepts input', async ({ page }) => {
    ...
    await expect(page.locator('.t-line', { hasText: 'Type "hdi" to get started' })).toBeVisible();
  ...
  test('cat README.md shows readme content', async ({ page }) => {
    ...
    await expect(page.locator('.t-line', { hasText: '# express-api' })).toBeVisible();
  });
  ...
  test('empty enter shows new prompt without error', async ({ page }) => {
    ...
    await expect(page.locator('.t-yellow', { hasText: 'Command not found' })).toHaveCount(0);
  });
});
