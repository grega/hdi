...
test.describe('Interactive picker', () => {
  ...
  test('renders with title, commands, and footer', async ({ page }) => {
    ...
    await expect(page.locator('.picker-wrap', { hasText: 'express-api' })).toBeVisible();
    await expect(page.locator('.picker-row.selected')).toHaveCount(1);
    await expect(page.locator('.picker-footer', { hasText: 'navigate' })).toBeVisible();
  });
  ...
  test('arrow down/up navigates between commands', async ({ page }) => {
    ...
    const second = await page.locator('.picker-row.selected .t-command').textContent();
  ...
  test('Tab jumps to next section', async ({ page }) => {
    ...
    const jumped = await page.locator('.picker-row.selected .t-command').textContent();
    expect(jumped).not.toBe(first);
    // Tab should skip past multiple commands, not just move one
    await page.keyboard.press('ArrowUp');
    const oneAbove = await page.locator('.picker-row.selected .t-command').textContent();
  ...
  test('Enter shows execute flash', async ({ page }) => {
    ...
    await expect(page.locator('.flash-execute', { hasText: 'would execute' })).toBeVisible();
  });

  test('c copies and shows flash', async ({ page, context }) => {
    await context.grantPermissions(['clipboard-read', 'clipboard-write']);
    await page.keyboard.press('c');
    await expect(page.locator('.flash-msg', { hasText: 'Copied' })).toBeVisible();
  });
